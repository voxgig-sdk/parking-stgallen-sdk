...
import (
	...
	omni "github.com/voxgig-sdk/parking-stgallen-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ParkingStgallenError); is {
				out := map[string]any{
					"name":    "ParkingStgallenError",
...
func omniCtx(arg any, client *sdk.ParkingStgallenSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ParkingStgallenSDK); is {
			client = live

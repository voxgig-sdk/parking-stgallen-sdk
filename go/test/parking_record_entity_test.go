...
func TestParkingRecordEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.ParkingRecord(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ParkingRecord(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		parkingRecordRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.parking_record")))
...
func parking_recordBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("PARKING_STGALLEN_TEST_PARKING_RECORD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PARKING_STGALLEN_TEST_PARKING_RECORD_ENTID"])

# parking-stgallen-cli

boru-driven command-line client **and** interactive REPL for the ParkingStgallen
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/parking-stgallen-cli)
make build

# 2. See usage (words, entities, env vars)
./parking-stgallen-cli --help

# 3. Provide credentials once, via the environment
export PARKING_STGALLEN_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./parking-stgallen-cli list parking_record
./parking-stgallen-cli load 1 parking_record            # {id:1} shorthand
./parking-stgallen-cli load '{id:1}' parking_record       # explicit match map

# 5. Override the API base URL for a single call
PARKING_STGALLEN_BASE=https://api.example.com ./parking-stgallen-cli list parking_record

# 6. No arguments -> interactive REPL
./parking-stgallen-cli
parking-stgallen> list parking_record
parking-stgallen> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/parking-stgallen-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/parking-stgallen-cli list parking_record
   ```
...
```sh
./parking-stgallen-cli load 1 parking_record          # scalar shorthand for {id:1}
./parking-stgallen-cli load '{id:1}' parking_record     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `parking-stgallen>`). Each line is
evaluated as its own boru expression:

```text
$ ./parking-stgallen-cli
parking-stgallen> list parking_record
parking-stgallen> /help
parking-stgallen> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `parking_record`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/parking-stgallen-cli`. |
...
(`./parking-stgallen-cli <expr>`) and interactively (the REPL), and expressions compose the

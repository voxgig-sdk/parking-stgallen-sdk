// Command ParkingStgallen-cli is a CLI + REPL for the ParkingStgallen SDK,
...
const prompt = "parking-stgallen"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "parking_record"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PARKING_STGALLEN_APIKEY (api key), PARKING_STGALLEN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PARKING_STGALLEN_APIKEY carries the API key and
	// PARKING_STGALLEN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PARKING_STGALLEN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PARKING_STGALLEN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewParkingStgallenSDK(opts)

...
// ParkingStgallen SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ParkingStgallenTestFeature extends ParkingStgallenBaseFeature
{
    ...
    public function init(ParkingStgallenContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ParkingStgallenContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ParkingStgallenParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ParkingStgallenContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ParkingStgallenContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ParkingStgallenParam::call($ctx, $k);

...
// ParkingStgallen SDK ParkingRecord entity
...
class ParkingRecordEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ParkingRecordEntity($this->_client, $opts);
    }

    /**
     * @param ParkingRecord|array $args ParkingRecord data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ParkingStgallenHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ParkingRecord|array The current ParkingRecord data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ParkingRecord fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ParkingStgallenHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ParkingRecord fields).
     */
    ...
    /**
     * Load a single ParkingRecord.
     *
     * @param ParkingRecordLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ParkingRecordLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ParkingRecord|array The loaded ParkingRecord as an assoc-array at the
     *   SDK boundary; throws ParkingStgallenError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ParkingStgallenHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List ParkingRecord items matching the given filter.
     *
     * @param ParkingRecordListMatch|array|null $reqmatch Match filter (any subset
     *   of ParkingRecord fields) as an assoc-array; ParkingRecordListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ParkingRecord[]|array A list of ParkingRecord items as assoc-arrays at
     *   the SDK boundary; throws ParkingStgallenError on failure (item-5 convention).
     */

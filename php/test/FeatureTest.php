...
// ParkingStgallen SDK feature test
//
...
class FtCtrl extends ParkingStgallenControl
{
...
class FtClient
{
    ...
    private ParkingStgallenUtility $utility;

    public function __construct(ParkingStgallenUtility $utility, array $options)
    {
    ...
    public function get_utility(): ParkingStgallenUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ParkingStgallenContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ParkingStgallenContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ParkingStgallenContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ParkingStgallenConfig::make_feature($name);
    ...
    private static function buildUrl(ParkingStgallenSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ParkingStgallenError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ParkingStgallenResponse($fetched);
        }
    ...
    private function fail(ParkingStgallenContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ParkingStgallenContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

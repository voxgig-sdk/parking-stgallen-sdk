# parking-stgallen-mcp

[MCP](https://modelcontextprotocol.io) server exposing the ParkingStgallen SDK as
two agent tools — `parking-stgallen_list` and `parking-stgallen_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/parking-stgallen-mcp)
make build

# 2. Provide credentials via the environment
export PARKING_STGALLEN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user parking-stgallen \
  -- /absolute/path/to/parking-stgallen-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./parking-stgallen-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// parking-stgallen_list: first page of records
{ "entity": "parking_record" }
{ "entity": "parking_record", "query": { } }

// parking-stgallen_load: one record by id
{ "entity": "parking_record", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/parking-stgallen-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user parking-stgallen \
     -- "$PWD"/dist/*/parking-stgallen-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `parking-stgallen_list` and `parking-stgallen_load` tools now appear
   in new sessions. Ask the agent to *"list parking_record using parking-stgallen"*
   and it calls `parking-stgallen_list` with `{"entity":"parking_record"}`.
...
```sh
export PARKING_STGALLEN_APIKEY=sk_live_xxx            # API key
export PARKING_STGALLEN_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `parking-stgallen_list` tool
...
### Call the `parking-stgallen_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `parking-stgallen_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `parking-stgallen_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PARKING_STGALLEN_APIKEY` | API key sent with every request. |
| `PARKING_STGALLEN_BASE` | Optional override of the API base URL. |
...
```sh
./parking-stgallen-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"parking-stgallen_load","arguments":{"entity":"parking_record","query":{"id":1}}}}'
```

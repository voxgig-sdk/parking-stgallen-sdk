...
module ParkingStgallenUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ParkingStgallenResponse.new({})
    ...
    if fetch_err
      ...
      response = ParkingStgallenResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ParkingStgallenResponse.new(fetched)
    else

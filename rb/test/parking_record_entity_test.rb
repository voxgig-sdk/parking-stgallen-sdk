...
class ParkingRecordEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = ParkingStgallenSDK.test(seed, nil)
    seen = base.ParkingRecord(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.ParkingRecord(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set PARKING_STGALLEN_TEST_PARKING_RECORD_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def parking_record_basic_setup(extra)
  ...
  entid_env_raw = ENV["PARKING_STGALLEN_TEST_PARKING_RECORD_ENTID"]

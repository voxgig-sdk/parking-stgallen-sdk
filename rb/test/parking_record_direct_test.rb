...
class ParkingRecordDirectTest < Minitest::Test
  def test_direct_list_parking_record
    setup = parking_record_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-parking_record", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_parking_record
    setup = parking_record_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-parking_record", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def parking_record_direct_setup(mockres)

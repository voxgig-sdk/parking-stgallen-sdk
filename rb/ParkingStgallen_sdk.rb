...
require_relative 'ParkingStgallen_types'
...
class ParkingStgallenSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ParkingStgallenHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ParkingStgallenHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ParkingStgallenFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ParkingStgallenUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ParkingStgallenHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ParkingStgallenHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ParkingStgallenHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ParkingStgallenError => err
    ...
    if fetched.is_a?(Hash)
      status = ParkingStgallenHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ParkingStgallenError.new(
        "graphql_error", "ParkingStgallenSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ParkingStgallenSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end

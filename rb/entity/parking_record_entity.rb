...
class ParkingRecordEntity
  ...
  def make
    opts = @_entopts.dup
    ParkingRecordEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ParkingStgallenHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ParkingRecord, Hash] the current ParkingRecord data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ParkingStgallenHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ParkingRecord fields)
  def match_get
  ...
  #
  # @param reqmatch [ParkingRecordLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ParkingRecord.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ParkingRecord, Hash] the loaded ParkingRecord; raises ParkingStgallenError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ParkingStgallenHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [ParkingRecordListMatch, Hash, nil] match filter (any subset of
  #   ParkingRecord fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<ParkingRecord>, Array] the matching ParkingRecord items; raises ParkingStgallenError on failure

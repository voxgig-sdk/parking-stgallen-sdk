# ParkingStgallen SDK test runner
...
class ParkingStgallenTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ParkingStgallenTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ParkingStgallenTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ParkingStgallenTestRunner.getenv("PARKING_STGALLEN_TEST_LIVE")
        override = ParkingStgallenTestRunner.getenv("PARKING_STGALLEN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ParkingStgallenTestRunner.getenv(key)
        ...
        explain = ParkingStgallenTestRunner.getenv("PARKING_STGALLEN_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ParkingStgallenTestRunner._test_control is not None:
            return ParkingStgallenTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ParkingStgallenTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ParkingStgallenTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ParkingStgallenTestRunner.load_test_control()
...
def load_env_local():
    ParkingStgallenTestRunner.load_env_local()


def env_override(m):
    return ParkingStgallenTestRunner.env_override(m)


def entity_data(v):
    return ParkingStgallenTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ParkingStgallenTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ParkingStgallenTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ParkingStgallenTestRunner.load_test_control()


def live_client_options():
    return ParkingStgallenTestRunner.live_client_options()


def live_delay_ms():
    return ParkingStgallenTestRunner.live_delay_ms()

...
from parkingstgallen_sdk.core.helpers import get_ctx_prop, to_map
...
class ParkingStgallenContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = ParkingStgallenControl()
        ...
        if isinstance(sp, ParkingStgallenSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, ParkingStgallenResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, ParkingStgallenResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return ParkingStgallenOperation({})
    ...
    def make_error(self, code, msg):
        return ParkingStgallenError(code, msg, self)

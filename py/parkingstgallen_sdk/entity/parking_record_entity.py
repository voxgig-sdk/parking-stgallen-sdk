...
from parkingstgallen_sdk.parkingstgallen_types import (
    ParkingRecord,
...
class ParkingRecordEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ParkingRecordEntity(self._client, opts)
    ...
    def data_get(self) -> ParkingRecord:
    ...
    def match_get(self) -> ParkingRecord:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ParkingRecord:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ParkingRecord().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ParkingRecord]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ParkingRecord().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",

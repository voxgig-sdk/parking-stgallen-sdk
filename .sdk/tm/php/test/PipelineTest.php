...
// ParkingStgallen SDK pipeline test
//
...
{
    private static function utility(): ParkingStgallenUtility
    {
        return new ParkingStgallenUtility();
    }
    ...
    private static function ctx(array $over = []): ParkingStgallenContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ParkingStgallenContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ParkingStgallenOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ParkingStgallenError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ParkingStgallenOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ParkingStgallenMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ParkingStgallenOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ParkingStgallenMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ParkingStgallenOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ParkingStgallenMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ParkingStgallenSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ParkingStgallenMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ParkingStgallenResponse([]),
            'result' => new ParkingStgallenResult([]),
        ]);
        [, $err] = ParkingStgallenMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ParkingStgallenSpec([]),
            'response' => null,
            'result' => new ParkingStgallenResult([]),
        ]);
        [, $err] = ParkingStgallenMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ParkingStgallenSpec(['step' => 's']),
            'response' => new ParkingStgallenResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ParkingStgallenResult([]),
        ]);
        [, $err] = ParkingStgallenMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ParkingStgallenSpec(['step' => 's']),
            'response' => new ParkingStgallenResponse(self::resp(200, ['v' => 1])),
            'result' => new ParkingStgallenResult([]),
        ]);
        [, $err] = ParkingStgallenMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ParkingStgallenSpec(['step' => 's']),
            'response' => new ParkingStgallenResponse(self::resp(200, ['v' => 2])),
            'result' => new ParkingStgallenResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ParkingStgallenMakeResponse::call($ctx);
    ...
    {
        $preset = new ParkingStgallenResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ParkingStgallenSpec([]),
            'response' => new ParkingStgallenResponse([]),
            'result' => new ParkingStgallenResult([]),
        ...
        [$got, $err] = ParkingStgallenMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ParkingStgallenResult([])]);
        [, $err] = ParkingStgallenMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ParkingStgallenSpec([]), 'result' => null]);
        [, $err] = ParkingStgallenMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ParkingStgallenSpec(['step' => 's']),
            'result' => new ParkingStgallenResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ParkingStgallenOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ParkingStgallenMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ParkingStgallenSpec(['step' => 's']),
            'result' => new ParkingStgallenResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ParkingStgallenOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ParkingStgallenMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ParkingStgallenResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ParkingStgallenSpec([]),
            'result' => new ParkingStgallenResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ParkingStgallenMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ParkingStgallenMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ParkingStgallenError('boom', 'boom');
        $utility->fetcher = function (ParkingStgallenContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ParkingStgallenSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ParkingStgallenMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ParkingStgallenContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ParkingStgallenMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ParkingStgallenResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ParkingStgallenMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ParkingStgallenContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ParkingStgallenSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ParkingStgallenMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ParkingStgallenResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ParkingStgallenSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ParkingStgallenMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ParkingStgallenMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ParkingStgallenMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ParkingStgallenResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ParkingStgallenDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ParkingStgallenResult(['ok' => false])]);
        $this->expectException(ParkingStgallenError::class);
        ParkingStgallenDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ParkingStgallenResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ParkingStgallenMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ParkingStgallenMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ParkingStgallenBaseFeature();
        $b = new ParkingStgallenBaseFeature();
        ParkingStgallenFeatureAdd::call($ctx, $a);
        ParkingStgallenFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ParkingStgallenBaseFeature
    {
        $f = new ParkingStgallenBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ParkingStgallenFeatureAdd::call($ctx, self::named_feature('a'));
        ParkingStgallenFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        ParkingStgallenFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        ParkingStgallenFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        ParkingStgallenFeatureAdd::call($ctx, $replace);
        ...
        ParkingStgallenFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ParkingStgallenContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ParkingStgallenSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ParkingStgallenPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        ParkingStgallenPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ParkingStgallenResponse(['headers' => null]),
            'result' => new ParkingStgallenResult([]),
        ]);
        ParkingStgallenResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ParkingStgallenResultBody::call($ctx);

...
// ParkingStgallen SDK retry feature
...
class ParkingStgallenRetryFeature extends ParkingStgallenBaseFeature
{
    ...
    public function init(ParkingStgallenContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ParkingStgallenContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ParkingStgallenContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ParkingStgallenContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

...
//     (\`new ParkingStgallenSDK(...)\` / \`ParkingStgallenSDK.test(...)\` become
//     \`ParkingStgallenSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ParkingStgallenSDK'
...
const TEST_SEED = {"entity":{"parking_record":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ParkingStgallenSDK(...)\` and any
// \`ParkingStgallenSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ParkingStgallenSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ParkingStgallenSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

...
describe('ParkingRecordDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new ParkingStgallenSDK({
  ...
  test('direct-load-parking_record', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-parking_record', setup.live)) return
  ...
  test('direct-list-parking_record', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-parking_record', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PARKING_STGALLEN_TEST_PARKING_RECORD_ENTID']
